# Hi, I'm Dwij Joshi 👋

**Mechatronics Engineering Student @ Toronto Metropolitan University**  
Expected Graduation: April 2029

I build at the intersection of hardware and software: embedded systems,
sensor integration, and electromechanical design.

---

## 🔧 What I Work With

| Category | Tools & Technologies |
|---|---|
| **Languages** | C++, C, Python |
| **Hardware** | Arduino, STM32, GPIO, UART, I2C, SPI |
| **Tools** | VS Code, PlatformIO, Git, MATLAB |
| **Lab Equipment** | Vernier Sensors, Oscilloscope, Multimeter |

---

## 🚧 Currently Building

- **Embedded GPIO Demo** — Bare-metal button/interrupt project on Arduino
  (no IDE abstractions)

---

## 📌 Featured Projects

| Project | Description | Stack |
|---|---|---|
| [RTOS Task Scheduler](https://github.com/dwijjoshi14/rtos-task-scheduler) | FreeRTOS 3-task system — queue, semaphore, mutex | C · FreeRTOS |
| [Heat Management System](https://github.com/dwijjoshi14/heat-management-system) | Arduino thermal control — TMP36, PWM fan, NeoPixel, LCD | C++, Arduino |
| [DC Circuit Analyzer](https://github.com/dwijjoshi14/dc-circuit-analyzer) | Applies Ohm's Law + KVL/KCL to series/parallel circuits | C++ |
| [Embedded GPIO Demo](#) | Bare-metal interrupt-driven I/O | C, Arduino |

---

## 📫 Connect

- 🔗 [LinkedIn] https://www.linkedin.com/in/dwij-joshi-14-tron/
- 🌐 Portfolio: [dwijjoshi14.github.io](https://dwijjoshi14.github.io)

---

*Ontario Scholar · TMU Mechatronics Engineering*
